...
router.get("/camp/:id", (req, res) => {
  ...
  var sql =
    "SELECT pic,title,subtitle,type,site,intro,phone,share_time FROM travel_camp WHERE id=?";
...
router.get("/v1/result/:id&:type",(req,res)=>{
  ...
  var sql = "";
  if(type == "0"){ 
    sql = "SELECT pic,title,subtitle,type,site,address,intro,phone,share_time FROM travel_camp WHERE id=?";//臻选表
  }else if(type == "1"){
    sql = "SELECT uid,pic,title,subtitle,likes,share_time FROM travel_shared WHERE id=?";//分享表
  }
  pool.query(sql,[id],(err,result)=>{
    if(err)throw err;
    if(result.length > 0){
